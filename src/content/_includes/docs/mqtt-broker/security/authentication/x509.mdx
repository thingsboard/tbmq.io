...
Instead of relying on a username and password, clients present a <LearnLink slug="mqtt-client-certificates">client certificate</LearnLink> during the TLS handshake, allowing the broker to authenticate them based on the Common Name (CN) certificate attribute.
...
The **Client authentication** setting controls whether the broker asks for — or insists on — a certificate from the client during the TLS handshake:

- **Requested** (default) — the broker requests a client certificate, but the TLS handshake still completes if the client does not present one. Suitable when X.509 is one of several enabled providers.
- **Required** — the client must present a certificate; otherwise the TLS handshake is aborted. This enforces true mutual (two-way) TLS.
...
The `pubAuthRulePatterns` and `subAuthRulePatterns` may also include the `${cn}` placeholder, which TBMQ substitutes at connection time with the client certificate's Common Name — enabling per-device rules such as `${cn}/data/.*`.
...
X.509 certificates are used to set up <a href="https://en.wikipedia.org/wiki/Mutual_authentication" target="_blank" rel="noopener noreferrer">mutual</a> (two-way) authentication for MQTT over TLS. Set **Client authentication** to **Required** (see [above](#client-authentication)) if the broker should reject connections that do not present a client certificate.
