...
SCRAM (Salted Challenge Response Authentication Mechanism) is a secure challenge-response authentication method that allows clients to prove their identity without transmitting the actual password over the network. It is one of the <LearnLink slug="mqtt-authentication">MQTT authentication methods</LearnLink> TBMQ supports, alongside Basic, X.509, JWT, and HTTP.
