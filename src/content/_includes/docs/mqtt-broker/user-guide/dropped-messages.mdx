...
A message published through the <DocLink product={props.product} path="publish-message">REST publish API</DocLink> has no client session behind it, so its reception drops are keyed by the reserved client ID `tbmq_rest_api`.
