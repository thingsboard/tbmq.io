...
|---|---|---|
| **Direct** — non-persistent sessions, plus persistent sessions receiving at QoS 0, regardless of <DocLink product={props.product} path="user-guide/mqtt-client-type">client type</DocLink> | `MQTT_MSG_WRITE_AND_FLUSH` | `true` — flush per message |
...
2. **Write without flush** — each outgoing PUBLISH is written with `channel.write()` and increments the buffer count.
   Only PUBLISH packets are buffered on these two paths: PUBREL, the second phase of a QoS 2 delivery, and retained
   messages sent in response to a SUBSCRIBE are always flushed immediately.
...
If sessions are evicted because the cache is full, the broker logs an `INFO` message —
`Client session was evicted due to cache size limit` — followed by the configured limit. Seeing it often means
`MQTT_BUFFERED_CACHE_MAX_SIZE` is below your concurrent subscriber count and should be raised.
...
2. A flush is triggered every `buffered-msg-count` messages (default: 10). The counter is local to the batch and
   restarts at zero on every poll, so a value larger than the batch — polls are capped by the consumer's
   `max.poll.records`, **200** by default for a client's own topic — never fires a flush of its own.
