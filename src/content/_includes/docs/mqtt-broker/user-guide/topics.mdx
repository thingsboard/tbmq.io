...
MQTT topics are the addressing mechanism that enables publishers and subscribers to exchange messages. Understanding how <LearnLink slug="topics">topics and wildcards</LearnLink> work is essential for designing efficient subscription patterns in your MQTT deployments.
...
In this case, TBMQ delivers the message **once**, using the subscription with the **highest QoS**.

## Configuring maximum topic segments

You can define the maximum number of segments (levels) allowed in a topic name or topic filter using the `MQTT_TOPIC_MAX_SEGMENTS_COUNT` environment variable. Segments are separated by forward slashes (`/`), so a topic with N slashes has N + 1 segments. Leading, trailing, and adjacent slashes produce empty segments, which count toward the limit.

By default, this is set to `0` (disabled — no restriction).

**Example:** Setting `MQTT_TOPIC_MAX_SEGMENTS_COUNT=3` causes the broker to reject topics with more than three segments. A topic like `sensors/floor1/temperature` (three segments) is accepted, while `sensors/floor1/room1/temperature` (four segments) is rejected.
