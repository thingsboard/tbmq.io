...
<Aside type="caution">
  When a shared subscription is deleted, the corresponding Kafka topic is also deleted. This requires <code>TB_KAFKA_ENABLE_TOPIC_DELETION=true</code> (the default); when disabled, TBMQ keeps the topic and simply stops using it.
</Aside>

<ShowFor product={props.product} show={[Products.TBMQ_PE]}>
The shared subscription details page also includes an **Audit logs** tab listing the <DocLink product={props.product} path="security/audit-log">audit log</DocLink> entries related to the entity.

<Aside type="caution">
  Adding, editing, and deleting shared subscriptions, as well as the Kafka topic actions, are available only to users
  with administrator rights — Viewers get a read-only view. The REST API enforces the same rule with the `READ`,
  `WRITE`, `CREATE`, and `DELETE` permissions. See
