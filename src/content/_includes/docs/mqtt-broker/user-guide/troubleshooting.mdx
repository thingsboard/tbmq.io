...
|---|---|---|
| Connect / CONNACK | `actors.client.service.connect` *(package)* | CONNACK send or connection-validation failures, topic-alias and keep-alive adjustments. Does **not** cover auth failures or malformed packets — see the next two rows |
| Authentication | `service.auth` *(package)* | CONNECTs are rejected and you need the reason: bad credentials, invalid JWT, untrusted certificate, failed HTTP auth. Covers all providers, and is the key logger for [Clients cannot connect](#clients-cannot-connect) |
| Channel / TLS errors | `server.MqttSessionHandler` | TLS handshake failures, protocol violations, malformed packets, connections dropped mid-handshake |
| Disconnect | `actors.client.service.disconnect.DisconnectServiceImpl` | You need the reason behind a specific client's disconnect |
| Subscriptions | `service.subscription.ClientSubscriptionConsumerImpl` | A subscription made on one node isn't reaching another — this consumer replicates subscription state across the cluster |
| Retained messages | `service.mqtt.retain.RetainedMsgConsumerImpl` | Retained messages aren't propagating to, or clearing from, other nodes |
| Kafka consumer lag | `queue.kafka.stats.TbKafkaConsumerStatsService` | You raised the root level to cut noise but still want the [lag line](#kafka-consumer-group-lag) — this logger's level controls it independently of root |
| Actor system | `actors.DefaultTbActorSystem` | Client actor and session lifecycle — actors being created and stopped, not message content |
| Stats printing | `service.stats` *(package)* | Keeps the periodic stats dump visible if you've turned the root level below INFO |
