...
| `incomingPublishMsg_consumed_total{statsName="tmpFailed"}` | Messages retried on the main ingest pipeline — not lost yet. |
...
- Scale broker nodes horizontally, or increase the relevant consumer count (`queue.msg-all.consumers-count`, `queue.device-persisted-msg.consumers-count`, and the equivalents for the other pipelines).
...
```promql
jvm_memory_used_bytes{area="heap"} / ignoring(id) group_left jvm_memory_max_bytes{area="heap"}
```
...
| `sqlQueue_total{statsName="failedMsgs"}` | Batches that failed to write to the database. |
...
| Health API (`/actuator/health`) | Whether `db`, `kafka`, and `redis` are each reachable from this node, right now. |
...
- Check the dependency's own health first — <DocLink product={props.product} path="other/health">Health API</DocLink> reports `db`, `kafka`, and `redis` as separate components, so a `DOWN` there tells you which one to chase before looking anywhere else.
