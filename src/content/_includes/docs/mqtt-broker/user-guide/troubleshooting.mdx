...
PromQL for the pack-latency p50 per consumer:

```promql
max by (consumerId) (incomingPublishMsg_consumed_pack_processing_time_seconds{quantile="0.5"})
```
...
Heap usage as a fraction of the ceiling, summed across the per-pool series (G1's Eden and Survivor pools report no fixed max, so the `> 0` filter keeps only the pool that does):
