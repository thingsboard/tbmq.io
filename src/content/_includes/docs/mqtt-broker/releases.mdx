...
- **Dropped Message Analytics:** Dropped messages are now persisted with a structured reason taxonomy and surfaced in the UI, so operators can see exactly why a message never reached its subscribers. Results can be rolled up by topic or by client, narrowed with exact-match modes, and filtered by client type. MQTT 5 topic aliases are resolved at drop time, so the recorded topic is always the real one.
