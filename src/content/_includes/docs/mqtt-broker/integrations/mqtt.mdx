...
| **Events topic name** | Topic on the external broker that [client lifecycle events](#client-lifecycle-events) are published to. Shown once at least one event type is selected, and required in that case. Cannot contain the `+` or `#` wildcards. |
