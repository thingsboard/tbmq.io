...
- Events are produced on the MQTT processing thread, so the broker never lets them delay a client. If Kafka cannot accept the record, the event is dropped and counted in the broker's `droppedLifecycleEvents` statistic instead of being retried.
- Events travel on their own per-integration topic (`tbmq.ie.event.$integrationId`) with a shorter retention than the message topic, so an event backlog never holds up message delivery.
...
| Environment variable | Default | Description |
|---|---|---|
| `TB_IE_EVENT_MSG_POLL_INTERVAL` | `1000` | Poll interval for `tbmq.ie.event` topics (ms). |
| `TB_IE_EVENT_MSG_PACK_PROCESSING_TIMEOUT` | `30000` | Processing timeout per event batch (ms). |
| `TB_IE_EVENT_MSG_ACK_STRATEGY_TYPE` | `SKIP_ALL` | Strategy: `SKIP_ALL` or `RETRY_ALL`. |
| `TB_IE_EVENT_MSG_ACK_STRATEGY_RETRIES` | `5` | Number of retries (0 = unlimited). Used with `RETRY_ALL`. |
| `TB_IE_EVENT_MSG_ACK_STRATEGY_PAUSE_BETWEEN_RETRIES` | `1` | Pause between retries (seconds). |
