...
| `restPublishMsgs_total` | counter | `result` = `accepted`, `quota_exceeded`, `failed` | Not a per-PUBLISH series: REST publish calls that reached the publish path. Note the `result` label |
...
`incomingPublishMsg_published_total` like any other publish. `quota_exceeded` is a refusal by the cluster-wide
...
This series is also the one exception to the `statsName` convention above — its outcome label is `result`.
