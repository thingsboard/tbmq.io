...
<ShowFor product={props.product} show={[Products.TBMQ_PE]}>
The `msgDownlink` row describes the 2.3 → 2.4 upgrade only. Since **2.4.1** the name is emitted again, with a meaning it never had before: it counts records a <DocLink product={props.product} path="integrations/kafka-source">Kafka source</DocLink> integration consumed from its external system. A 2.3 dashboard panel reading it is still wrong — it was measuring nothing then — but on 2.4.1 and later the series is no longer absent.
</ShowFor>
