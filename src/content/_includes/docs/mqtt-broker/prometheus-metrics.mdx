...
<Aside type="caution">
  `METRICS_ENDPOINTS_EXPOSE` replaces the whole list rather than adding to it, so setting it to `prometheus` alone takes
  `/actuator/health` down with it — see <DocLink product={props.product} path="other/health">Health API</DocLink>.
</Aside>
...
| `restPublishMsgs_total` | counter | `result` = `accepted`, `quotaExceeded`, `failed` | Not a per-PUBLISH series: REST publish calls that reached the publish path |
...
`restPublishMsgs_total` sits in front of that pipeline: it counts calls to the
<DocLink product={props.product} path="mqtt-message-publishing">REST publish API</DocLink> rather than MQTT PUBLISH
packets, and a call rejected before it reaches the publish path — on authorization or request validation — is not
counted at all. `accepted` means the publish queue took the message, which then continues through
`incomingPublishMsg_published_total` like any other publish. `quotaExceeded` is a refusal by the cluster-wide
throughput quota, `failed` a queue rejection. The tag follows the queue's own answer, so a call that timed out with
`503` still counts `accepted` if the queue acknowledged it afterwards.

All three outcomes can move `droppedMsgs_total`, `accepted` included: a publish accepted with no matching
subscription is counted there like any other no-subscriber PUBLISH. Publishing to topics nothing subscribes to is a
normal use of the API and answers `202`, so on a deployment that does it routinely `droppedMsgs_total` climbs
one-for-one without anything being lost.
...
`throughputQuotaDegraded_total` increments every time the cluster-wide throughput quota fails to draw from Redis.
Within the degraded grace the quota keeps granting — traffic passes **unmetered** and nothing lands in
`droppedMsgs_total` — so inside that window this counter is the only sign that enforcement has stopped. Past the grace
the node refuses PUBLISH packets instead, and those refusals *do* land in `droppedMsgs_total`.

The series is registered on every node whether or not the quota is enabled, so a flat zero is the normal reading. Draws
only run when traffic charges the quota, so the counter stays flat on an idle node too: alert on Redis reachability and
treat this counter as corroboration, not as the outage alarm.

<ShowFor product={props.product} show={[Products.TBMQ]}>
  The quota itself is described in <DocLink product={props.product} path="user-guide/backpressure">Backpressure</DocLink> — alert on this counter if you rely on it as a hard ceiling.
</ShowFor>
...
`ie_uplink_published_total` is data an integration publishes upstream into TBMQ. `integrationProcessor_total` and `integrationEventProcessor_total` cover per-integration message and lifecycle-event processing respectively, using the same `tmp*` vs. plain split as [Ingest pipeline](#ingest-pipeline) above. `integration_stats_counter_total` reports integration lifecycle and message activity: `name` identifies the metric being reported, not the integration's own name; `type` is the `IntegrationType`; and `state` is `success` or `failed`. `integration_stats_gauge` only ever carries `name="start"`: it reports the current count of integrations of each `type` that are running (`state="success"`) versus failed (`state="failed"`), not a per-event breakdown.

<ShowFor product={props.product} show={[Products.TBMQ]}>
Observed `name` values on `integration_stats_counter_total` are `start`, `stop` and `msgUplink`.
</ShowFor>

<ShowFor product={props.product} show={[Products.TBMQ_PE]}>
Observed `name` values on `integration_stats_counter_total` are `start`, `stop`, `msgUplink` and `msgDownlink`. Direction is from the Integration Executor's point of view: an outbound integration counts every delivery to its external system as `msgUplink`, and a <DocLink product={props.product} path="integrations/kafka-source">Kafka source</DocLink> integration counts every record it consumes as `msgDownlink` — so `type="KAFKA_SOURCE"` appears only under `msgDownlink`, and the other types only under `msgUplink`.

One more series belongs to the same family:

| Series | Type | Labels |
|---|---|---|
| `ie_source_published_total` | counter | `statsName` = `totalMsgs`, `successfulMsgs`, `failedMsgs` |

`ie_source_published_total` is the executor-side producer for `tbmq.ie.source`, the topic that carries sourced publishes into the broker — the inbound counterpart of `ie_uplink_published_total`. It is registered on the Integration Executor whether or not any source integration exists, so a flat zero is the normal reading on a deployment with none.
</ShowFor>
...
| `integration_stats_counter_total{name="msgDownlink"}` | Removed in 2.4 — the series was never emitted |
| `processedBytes` historical key | Removed — was never incremented |

<ShowFor product={props.product} show={[Products.TBMQ_PE]}>
The `msgDownlink` row ends there only for 2.4. Since **2.4.1** the name is emitted again, with a meaning it never had before: it counts records a <DocLink product={props.product} path="integrations/kafka-source">Kafka source</DocLink> integration consumed from its external system. A 2.3 dashboard panel reading it is still wrong — it was measuring nothing then — but on 2.4.1 and later the series is no longer absent.
</ShowFor>
