...
| `restPublishMsgs_total` | counter | `result` = `accepted`, `quota_exceeded`, `failed` | REST publish calls that reached the publish path |
...
<DocLink product={props.product} path="publish-message">REST publish API</DocLink> rather than MQTT PUBLISH
packets. `accepted` means the publish queue took the message — it then continues through
`incomingPublishMsg_published_total` like any other publish. `quota_exceeded` is a refusal by the total incoming
throughput quota, `failed` a queue rejection; both also count toward `droppedMsgs_total`.
