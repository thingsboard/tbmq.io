...
| Series | Type | Labels | Notes |
|---|---|---|---|
| `throughputQuotaDegraded_total` | counter | `cause` = `redis` | Times the cluster-wide throughput quota failed open because Redis was unreachable |

`throughputQuotaDegraded_total` is the inverse signal of `droppedMsgs_total`: it increments when the cluster-wide
throughput quota cannot reach Redis and therefore stops enforcing, letting traffic through **unmetered** rather than
refusing it. The series is registered on every node whether or not the quota is enabled, so a flat zero is the normal
reading — any increase means enforcement is silently off.
