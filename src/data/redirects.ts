...
// /products/…/ (2026-08) with no redirects — a deliberate decision, not an
// oversight: the site was not yet public, so no external links to the old
// paths exist, and thingsboard.io's edge table keeps its own legal pages on
// thingsboard.io (only /products/mqtt-broker/ itself points here, at the root),
// so nothing upstream resolves to the old URLs either.
