...
Markdown runs on **Sätteri**, Astro 7's Rust pipeline (`markdown.processor` in `astro.config.ts`), not remark/rehype. Sätteri plugins are not unified transformers: each declares visitor subscriptions (`element`, `mdxJsxFlowElement`, `mdxjsEsm`, …) with a `filter` array — an **empty** filter matches every node of that type — and gets its own document-order pass. Ordering matters: user plugins all run *before* Sätteri's built-in `heading-ids`, so anything that needs the finished heading list has to hook `ctx.data.astro.headings` rather than read it. Per-document state belongs on `ctx.data` (fresh per page); the plugin objects themselves are reused across pages.
