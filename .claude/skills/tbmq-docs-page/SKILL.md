...
- Conditional **headings** use `<ConditionalHeading level={2|3} id="…" showFor="…">`, not `##` — the TOC plugin (`config/plugins/satteri-mdx-include-headings.ts`) reads the raw tag to decide whether the heading enters that edition's TOC.
